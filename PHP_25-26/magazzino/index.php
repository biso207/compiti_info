...
// totale prodotti + numero pagine (coerenti col filtro!)
$total = countProducts($status, $q);
$pages = (int) max(1, ceil($total / $perPage));
$page  = clamp_int($page, 1, $pages);

// prodotti della pagina
$rows = getProducts($page, $perPage, $status, $q);
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarMain">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item"><a class="nav-link" href="#">Movimenti</a></li>
            </ul>

            <!-- SEARCH: usa GET così resta in url e la paginazione può mantenerla -->
            <form class="d-flex" role="search" method="get" action="index.php">
                <input type="hidden" name="status" value="<?= h($status) ?>">
                <input type="hidden" name="page" value="1">
                <input class="form-control me-2" name="q" type="search"
                       value="<?= h($q) ?>"
                       placeholder="Cerca SKU, nome, descrizione o categoria..."
                       aria-label="Search">
                <button class="btn btn-outline-light" type="submit">Cerca</button>
                <?php if ($q !== ""): ?>
                    <a class="btn btn-outline-secondary ms-2" href="index.php?status=<?= urlencode($status) ?>&page=1">Reset</a>
                <?php endif; ?>
            </form>
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <div>
                <small class="text-muted">
                    ...
                    <?php if ($q !== ""): ?>
                        <span class="ms-2">| Ricerca: "<b><?= h($q) ?></b>"</span>
                    <?php endif; ?>
                </small>
            </div>

            <div class="btn-group btn-group-sm">
                <!-- bottoni status: mantengono q -->
                <a class="btn btn-outline-secondary <?= $status==='all' ? 'active' : '' ?>"
                   href="index.php?status=all&q=<?= urlencode($q) ?>&page=1">Tutto</a>

                <a class="btn btn-outline-secondary <?= $status==='low' ? 'active' : '' ?>"
                   href="index.php?status=low&q=<?= urlencode($q) ?>&page=1">Sotto scorta</a>

                <a class="btn btn-outline-secondary <?= $status==='out' ? 'active' : '' ?>"
                   href="index.php?status=out&q=<?= urlencode($q) ?>&page=1">Esauriti</a>
            </div>
        </div>
        ...
        <div class="card-footer bg-white d-flex justify-content-between align-items-center">
            ...
            <nav aria-label="Page navigation">
                <?= renderPagination($page, $pages, $status, $q) ?>
            </nav>
        </div>
    </div>
</div>
